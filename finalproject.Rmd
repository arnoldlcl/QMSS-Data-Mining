...
Several of our selected features in the training set contain missing data. bartMachine had built-in methods for dealing with missing data, but other models don't. Using the `mice` package, the number of the missing values can be counted and visualized as follows.
...
Fitting logistic regression on the imputed training data using a classification threshold of 0.5 performs at baseline. Lowering the classification threshold to 0.2 gives worse results, but produces 6 true positives.  

####Random Forest

Another example of ensemble learning using trees, like BART above. The default arguments (500 trees, etc.) are used.   
...
Similarly to BART, both the training and testing data are akin to predicting that nobody donates.
