...
logit3 <- glm(as.factor(y) ~ loan_amnt + revol_bal + unemployed + inq_last_6mths + small_business, data = training, family = binomial)
...
X_test <- model.matrix(y ~ loan_amnt + revol_bal + unemployed + inq_last_6mths + small_business, data = testing)
y <- training$y
path1 <- glmpath(X, y, nopenalty.subset = 1, family = binomial(link = "logit"))
summary(path1) # Step 9 has the lowest AIC
y_hat_path1 <- predict(path1, newx = X_test, type = "response", s = 9)
cl_path1 <- as.integer(y_hat_path1 > 0.5)
table(testing$y, cl_path1)
```

So is this model. I also tested dozens of models with different variables, most of which either performed at baseline or worse than baseline. Random forests didn't get me anywhere either. I'll just take logit3 to be the best model. (Honestly, though, maybe we should have the full Lending Club data instead of just a subset of features...)
